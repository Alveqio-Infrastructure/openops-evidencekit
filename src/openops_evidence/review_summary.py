...
def _highlights(metrics: dict[str, Any]) -> list[str]:
    ...
    if metrics["access_warnings"]:
        highlights.append(f"{metrics['access_warnings']} access exposure warning(s) need review.")
    if metrics["monitoring_failures"]:
    ...
    if metrics["exposure_failures"]:
        highlights.append(f"{metrics['exposure_failures']} network exposure check(s) failed.")
    if metrics["exposure_warnings"]:
